...
public class LevelGrid : MonoBehaviour
{
    ...
    //Used to generate a serializable grid of LevelTiles
    [System.Serializable]
    public class Column
    {
    ...
    //Used when looking for a path between two positions
    ...
    */
    ...
    {
        if (_levelTile == null)
            Debug.LogError("No LevelTile Template found, be sure to add a LevelTile Template to the LevelGrid");
        if (_levelLocator == null)
            Debug.LogError("No LevelLocator found, be sure to add a transform to the LevelGrid so all the tiles can be placed there");
    ...
    #region SIMPLE GET SET FUNCTIONS
    ...
    #region COMPLEX GET FUNCTIONS
    //Search on the entire grid the closest tile to the position that has no unit
